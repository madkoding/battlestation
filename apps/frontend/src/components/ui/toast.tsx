...
>(({ className, ...props }, ref) => (
    <ToastPrimitives.Viewport
      ref={ref}
      className={cn(
        "fixed bottom-0 left-1/2 z-[100] flex max-h-screen w-[min(92vw,360px)] -translate-x-1/2 flex-col p-2 sm:left-auto sm:right-0 sm:w-full sm:max-w-[420px] sm:translate-x-0 sm:p-4",
        className
      )}
      {...props}
    />
...
const toastVariants = cva(
  "group pointer-events-auto relative flex w-full items-center justify-between space-x-3 sm:space-x-4 overflow-hidden rounded-md border p-3 pr-6 sm:p-6 sm:pr-8 shadow-lg transition-all data-[swipe=cancel]:translate-x-0 data-[swipe=end]:translate-x-[var(--radix-toast-swipe-end-x)] data-[swipe=move]:translate-x-[var(--radix-toast-swipe-move-x)] data-[swipe=move]:transition-none data-[state=open]:animate-in data-[state=closed]:animate-out data-[swipe=end]:animate-out data-[state=closed]:fade-out-80 data-[state=closed]:slide-out-to-right-full data-[state=open]:slide-in-from-bottom-full",
  {
    variants: {
      variant: {
        default: "border bg-bg-card text-text-primary",
        destructive:
          "destructive group border-accent-danger bg-accent-danger text-text-inverse",
        success: "border-success bg-bg-card text-text-primary",
        warning: "border-warning bg-bg-card text-text-primary",
        info: "border-accent-primary bg-bg-card text-text-primary",
      },
    },
    defaultVariants: {
      variant: "default",
